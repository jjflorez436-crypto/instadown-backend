...
def download_video():
    ...
    if fmt == "mp3":
        ydl_opts = {"format": "bestaudio/best", "outtmpl": output_path, "quiet": True, "postprocessors": [{"key": "FFmpegExtractAudio", "preferredcodec": "mp3", "preferredquality": "192"}]}
        final_ext = "mp3"
        mime = "audio/mpeg"
    else:
        ydl_opts = {"format": "bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best", "outtmpl": output_path, "quiet": True}
        final_ext = "mp4"
        mime = "video/mp4"
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
